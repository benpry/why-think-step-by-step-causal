...
def main(args):
    ...
    if args.only_selected_vars:
        df_pairs = pd.read_csv(
            here(
                f"data/training-data/selected-pairs/selected-pairs-net-{args.net_idx}.csv"
            )
        )
        chosen_pairs = set()
    ...
    for target, condition in chosen_pairs:
        ...
        for condition_val in (0, 1):
            for intervention in (True, False):
                ...
                prefix = "###\ntarget: {target}\n"
                if intervention:
                    prefix += f"do({condition}={condition_val})"
                else:
                    prefix += f"{condition}={condition_val}"
                prefix += f"{target}="
...
if __name__ == "__main__":
    ...
    df.to_csv(
        here(
            f"data/evaluation/causal/base-model-{args.base_model_name}/fixed-gen-probabilities-{model_name}.csv"
        )
    )
